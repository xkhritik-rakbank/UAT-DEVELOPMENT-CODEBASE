		...
		try {
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0) {
				for (int i = 0; i < fetchWorkitemListCount; i++) {
					...
					// Hritik - 18/07/23 -- ETB 
					try{
						String StrExttable_query = "SELECT WI_NAME,NTB,isnull(Etb_new,'') as 'Etb_new' FROM NG_DCC_EXTTABLE with(nolock) WHERE WI_NAME='" + processInstanceID + "'";
						...
						if (mainCode!=null && !"".equalsIgnoreCase(mainCode) && mainCode.equalsIgnoreCase("0")  && iTotalrec > 0){
							...
							DCC_CardClosureLog.DCC_CardClosureLogger.debug("Etb_new: " + Etb_new);
						}
