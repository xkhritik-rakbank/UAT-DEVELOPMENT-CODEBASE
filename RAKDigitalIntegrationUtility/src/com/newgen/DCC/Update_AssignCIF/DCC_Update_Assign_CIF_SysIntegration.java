...
import com.newgen.DPL.docGeneration.DocGen;
...
public class DCC_Update_Assign_CIF_SysIntegration implements Runnable {
	...
	private static String dplqueueID = "";
	...
	public void run()
	{
		...
		try
		{
			...
			dplqueueID = ConfigParamMap.get("dplqueueID");
			DCC_UpdateAssignCIFLog.DCC_Update_And_Assign_CIF_Logger.debug("dplqueueID: " + dplqueueID);

			final String ws_name = ConfigParamMap.get("WS_NAME");
			DCC_UpdateAssignCIFLog.DCC_Update_And_Assign_CIF_Logger.debug("WS_NAME: " + ws_name);

			final String Queuename = ConfigParamMap.get("QueueName");
			DCC_UpdateAssignCIFLog.DCC_Update_And_Assign_CIF_Logger.debug("QueueName: " + Queuename);
			...
			else
			{
				...
				while(true)
				{
					...
					DocGen DocumentGeneration=new DocGen();
					DocumentGeneration.startDPLDocumentUtility(cabinetName, jtsIP, jtsPort,sessionID,dplqueueID,socketDetailsMap,ConfigParamMap);
					System.out.println("DPL No More workitems to Process, Sleeping!"); //rubi
		...
		try
		{
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
			{
				for(int i=0; i<fetchWorkitemListCount; i++)
				{
					...
					String DB_Query = "SELECT NTB,isnull(Etb_new,'') as 'Etb_new',Product,IS_virtual_Card_Created,Is_CIF_ASSIGNED,CardOps_Reschedule,Is_CIF_UPDATED,FIRCO_Flag,EFMS_Status,FTS_Ack_flg,is_stp,Dectech_Decision,FircoUpdateAction,UW_Decision,Product,Preferred_Language,Nationality,FATCA_Tin_Number FROM NG_DCC_EXTTABLE with(nolock) WHERE WI_NAME='" + processInstanceID + "'";
			        ...
			        String Etb_new= xmlParserData.getValueOf("Etb_new"); // Hritik sprint 20-27
			        
			        DCC_UpdateAssignCIFLog.DCC_Update_And_Assign_CIF_Logger.error("Etb_new: "+Etb_new);
			        DCC_UpdateAssignCIFLog.DCC_Update_And_Assign_CIF_Logger.error("IS_virtual_Card_Created: "+IS_virtual_Card_Created);
			        
			        if(!"Y".equalsIgnoreCase(IS_virtual_Card_Created) && ("true".equalsIgnoreCase(NTB) || ("false".equalsIgnoreCase(NTB) && "Y".equalsIgnoreCase(Etb_new))))
					{
			        ...
			        ||(!"Y".equalsIgnoreCase(CardOps_Reschedule) && !"Y".equalsIgnoreCase(Is_CIF_UPDATED) && !"Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && !"Y".equalsIgnoreCase(IS_virtual_Card_Created) && !"Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && "true".equalsIgnoreCase(NTB)))
			        {
			        	...
			        	if("Y".equalsIgnoreCase(IS_virtual_Card_Created))
			        	{
				        	...
				        	|| "Confirmed Fraud".equalsIgnoreCase(efms) || "Negative".equalsIgnoreCase(efms) || ("Y".equalsIgnoreCase(stp)&&"R".equalsIgnoreCase(DectechDecision)) 
			        ...
			        //changes to execute service maintenance call only for ntb cases 
			        if("true".equalsIgnoreCase(NTB)) {
			        	Is_CIF_ASSIGNED="Y";
			        }
			        else if(("false".equalsIgnoreCase(NTB) && "Y".equalsIgnoreCase(Etb_new))) {
			        	Is_CIF_ASSIGNED="";// DCC Sprint 20-27 Hritik
			        }
			        
			        if (("Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && !"Y".equalsIgnoreCase(CardOps_Reschedule) && "true".equalsIgnoreCase(NTB))
			        	|| (!"Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && !"Y".equalsIgnoreCase(CardOps_Reschedule) && "false".equalsIgnoreCase(NTB) && "Y".equalsIgnoreCase(Etb_new))) {
			        //is_service_main
					...
					if (statuses != null && statuses.length > 0 && statuses[0].equalsIgnoreCase("0000"))
					{
						/**Update is_service_main value in exttable to Y**/
			        ...
			        ||(!"Y".equalsIgnoreCase(Is_CIF_UPDATED) && !"Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && !"Y".equalsIgnoreCase(CardOps_Reschedule))&& !"true".equalsIgnoreCase(NTB) && "".equalsIgnoreCase(Etb_new)
			        ||(!"Y".equalsIgnoreCase(Is_CIF_UPDATED) && !"Y".equalsIgnoreCase(Is_CIF_ASSIGNED) && !"Y".equalsIgnoreCase(CardOps_Reschedule))&& !"true".equalsIgnoreCase(NTB) && "Y".equalsIgnoreCase(Etb_new)) { // pipeline ETB handle PDSC - 1435
						...
						else if(docGenStatus.contains("Success"))
						{
							...
							// Hritik --- 19.7.23  -- ETB Case // pipeline ETB handle PDSC - 1435	
				        	if(!"true".equalsIgnoreCase(NTB) && "".equalsIgnoreCase(Etb_new)){
