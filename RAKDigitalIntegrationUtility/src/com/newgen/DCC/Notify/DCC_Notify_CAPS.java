...
public class DCC_Notify_CAPS {
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			String CreditStatus="";
			String genBridge="";
			String Service_type=""; //HRITIK 10.07.2024
			...
			String DBQuery = "SELECT CIF,Prospect_id,NOTIFY_CAPS_STATUS,ECRN,CRN,RM_Code,Credit_Shield_Flag,FinalTAI,Net_salary1_date,FinalDBR,Final_Limit,cast(SUBSTRING(Net_salary1_date,9,2)as int)+2 as salary_day,delegation_authority as ApprovalLevelCode,"+
							" (select Description from NG_MASTER_DCC_DELEGATION_AUTHORITY with (nolock) where Code=delegation_authority) as delegation_authority , deviation_description,bureau_reference_number,CURR_WSNAME,Product,Service_type,ApprovedLimit,CLI_STP,AECB_REF_NUM FROM NG_DCC_EXTTABLE with(nolock) WHERE WI_NAME='"+processInstanceID+"'";
			...
			if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec>0)
			{
				...
				ApprovedLimit=xmlParserData.getValueOf("ApprovedLimit");
				CLI_STP=xmlParserData.getValueOf("CLI_STP");
				
				DCC_Notify_CAPSLog.debug("Notify CAPS data Service_type: "+ Service_type);
				DCC_Notify_CAPSLog.debug("Notify CAPS data ApprovedLimit: "+ ApprovedLimit);
				DCC_Notify_CAPSLog.debug("Notify CAPS data CLI_STP: "+ CLI_STP);
				...
				else if("Sys_Limit_Increase".equalsIgnoreCase(CURR_WSNAME))
				{
					...
					genBridge="Y";
					if("CLI".equalsIgnoreCase(Service_type)) { // HRITIK 04.07.2024 
						sInputXML=sInputXML.replace("#SecurityChequeDetails#", "");
					}
					else{
			...
			// HRITIK - 04.07.2024 - CLI
			String bureau_reference_number ="";
			bureau_reference_number=validateValue(xmlParserData.getValueOf("bureau_reference_number"));
			if("".equalsIgnoreCase(bureau_reference_number)){
				bureau_reference_number=validateValue(xmlParserData.getValueOf("AECB_REF_NUM"));
			}
		...
		//	sInputXML=sInputXML.replace("#Final_Limit#", validateValue(xmlParserData.getValueOf("Final_Limit"))); -- Commented by Hritik for CLI Condition
			...
			// HRITIK 04.07.2024 PDSC-1856
			if("CLI".equalsIgnoreCase(Service_type) && "Y".equalsIgnoreCase(CLI_STP)) {
				sInputXML=sInputXML.replace("#Final_Limit#", validateValue(xmlParserData.getValueOf("ApprovedLimit")));
			}
			else if("CLI".equalsIgnoreCase(Service_type) && "N".equalsIgnoreCase(CLI_STP)){
			...
			{
				String responseXML=socketConnection( cabinetName,  username,  sessionId,  sJtsIp, iJtsPort,  processInstanceID,  ws_name, socket_connection_timeout,  integrationWaitTime, socketDetailsMap,  sInputXML);
