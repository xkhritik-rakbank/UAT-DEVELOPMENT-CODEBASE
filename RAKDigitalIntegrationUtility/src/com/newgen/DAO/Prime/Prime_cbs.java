   ...
   // private File exceFile;
	...
	public void run()
	{
		...
		try
		{
			...
			//get DPL QueueId for prime added by rubi
			String dplQueueID = DAOPrimeConfigParamMap.get("queueID");
			DAO_prime.DAOPrimeLogger.debug("QueueID: " + dplQueueID);
			
			String dplExcelFilePath=DAOPrimeConfigParamMap.get("DPLExcelFilePath");
			DAO_prime.DAOPrimeLogger.debug("ExcelFilePath: "+ExcelFilePath);
	            ...
	            for (int row = 1; row < rows-1; row++)
	            {
	            	...
	            	*/
	    			...
	    			while(read.hasNextLine())
	    			{
						...
						Card_Serno_DAO = col_val_ecrn[11].replace("'", ""); //changes done for Prime4 to pick card serno no instead of ECRN no.
	    				...
	    				DAO_prime.DAOPrimeLogger.debug("ReadPrimeFile Card_Serno_DAO: "+Card_Serno_DAO);

	    				// Ap select for getting Ecrn number check and insert flag
	    				String ECRN="",Wi_name="", Query_for_ecrn="",DCC_CardSerno="";
	    				if(str_Card_Type.equalsIgnoreCase("CREDIT")){
	    					Query_for_ecrn = "select top 1 ecrn as 'ECRN', WI_name as 'Wi_name', Card_Serno as 'DCC_CardSerno'  from NG_DCC_EXTTABLE with(nolock) where ECRN = '"+ECRN_dcc+"' or card_serno = '"+Card_Serno_DAO+"'";
	    				}
//	    				//add new condition for DPL
//	    				else if(str_Card_Type.equalsIgnoreCase("LOAN")){
//	    					Query_for_ecrn = "select top 1 ecrn as 'ECRN', WIName as 'Wi_name' from NG_DPL_EXTTABLE with(nolock) where ECRN = '"+ECRN_dcc+"'";
//	    				}
	    				else{
	    					
	    				// 	Commented bcoz comflict in the ECRN and SERNO - 12.06.2024
	    				//	Query_for_ecrn = "select top 1 ecrn as 'ECRN', WI_name as 'Wi_name'  from NG_DAO_WI_UPDATE with(nolock) where ecrn = '"+elite_crn+"' or ecrn = '"+Card_Serno_DAO+"'";
	    					
	    					Query_for_ecrn = "select top 1 ecrn as 'ECRN', WI_name as 'Wi_name'  from NG_DAO_WI_UPDATE with(nolock) where ecrn = '"+Card_Serno_DAO+"'";
	    				...
	    				if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec>0){
	    					...
	    					for (; objWorkList.hasMoreElements(true); objWorkList.skip(true)){
	    						...
	    						if(str_Card_Type.equalsIgnoreCase("CREDIT")){
		    						DCC_CardSerno= objWorkList.getVal("DCC_CardSerno").trim();	    							
	    						}

	    					}
	    					DAO_prime.DAOPrimeLogger.debug(" Wi_name: "+Wi_name+" for  ECRN or DAO Serno: "+ ECRN + " DCC Serno: "+DCC_CardSerno);
	    				}else{
	    					DAO_prime.DAOPrimeLogger.debug("Check for DPL");
	    					Query_for_ecrn = "select top 1 ecrn as 'ECRN', WIName as 'Wi_name' from NG_DPL_EXTTABLE with(nolock) where ECRN = '"+Card_Serno_DAO+"'";
	    					extTabDataIPXML=CommonMethods.apSelectWithColumnNames(Query_for_ecrn, cabinetName,sessionID);
		    				DAO_prime.DAOPrimeLogger.debug("Output_Query_for_ecrn: "+extTabDataIPXML);
		    				...
		    				if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec>0){
		    					...
		    					DAO_prime.DAOPrimeLogger.debug("ECRN not available in CARD/DEBIT/LOAn");
		    				}
	    				}
	    				...
	    				//insert NG_DPL_PRIME_COURIER for DPL
	    				else if(str_Card_Type.equalsIgnoreCase("LOAN")){
	    					apInsertInputXML=CommonMethods.apInsert(cabinetName, sessionID, prime_cloumn_name, s_prime,"NG_DPL_PRIME_COURIER");
	    				}
	    				else{
	    				...
	    				if(apInsertMaincode.equalsIgnoreCase("0")){
	    					...
	    					//added by rubi
	    						...
	    						updateExternalTable("NG_DPL_EXTTABLE","Courier_Flag","'File_received'","WIName='"+Wi_name+"'", sJtsIp, iJtsPort, cabinetName);
	    					}
	    					else{
								//if(ECRN.equals(elite_crn) || ECRN.equals(Card_Serno_DAO)){ comment bcz of conflict with elite_crn  - 12.06.2024
	    							if(ECRN.equals(Card_Serno_DAO)) {
	    					...
	    					if(str_Card_Type.equalsIgnoreCase("CREDIT")){
	    						...
	    						updateExternalTable("NG_DPL_EXTTABLE","Courier_Flag","'File_Error'","WIName='"+Wi_name+"'", sJtsIp, iJtsPort, cabinetName);
	    					}
	    					else{
			 ...
			 DAO_prime.DAOPrimeLogger.debug("Start entrydate "+entrydate);
				...
				while(read.hasNextLine())
				{
					...
					System.out.println("input " + insert_val);
				 	...
				 	String Query_for_ecrn = "";
				 	
				 	Query_for_ecrn = "select ChequeBk_ref as 'ChequeBk_ref', WI_name as 'WI_name' from NG_DAO_WI_UPDATE with(nolock) where ChequeBk_ref='"+chqBk_ref+"'";
					...
					int iTotalrec=Integer.parseInt(xmlParserData.getValueOf("TotalRetrieved"));
					DAO_prime.DAOPrimeLogger.debug("External table xmlParserData: "+xmlParserData);
					...
					if(xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec>0)
					{
						...
						//check chequeBk_ref no DPL table
						Query_for_ecrn = "select ChequeBk_ref as 'ChequeBk_ref', WIName as 'WI_name' from NG_DPL_EXTTABLE with(nolock) where ChequeBk_ref='"+chqBk_ref+"'";
						
						extTabDataIPXML=CommonMethods.apSelectWithColumnNames(Query_for_ecrn, CommonConnection.getCabinetName(), sessionID);
						DAO_prime.DAOPrimeLogger.debug("Output_Query_for_ecrn dpl: "+extTabDataIPXML);
						
						String extDPLTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,CommonConnection.getJTSIP(),CommonConnection.getJTSPort(),1);
						DAO_prime.DAOPrimeLogger.debug("DPL extTabDataOPXML : prime "+ extTabDataOPXML);
						
						XMLParser xmlDPLParserData= new XMLParser(extDPLTabDataOPXML);		
						
						if(xmlDPLParserData.getValueOf("MainCode").equalsIgnoreCase("0"))
						{
							String xmlDataExtTab=xmlDPLParserData.getNextValueOf("Record");
							xmlDataExtTab =xmlDataExtTab.replaceAll("[ ]+>",">").replaceAll("<[ ]+", "<");
							
							NGXmlList objWorkList=xmlDPLParserData.createList("Records", "Record");
							...
							DAO_prime.DAOPrimeLogger.debug("DPL Wi_name "+WI_name+" for  ChequeBk_ref "+ ChequeBk_ref+ " and account No: "+account_no);
						}
						//end dpl ap select
					...
					if(WI_name.contains("DigitalPL")){
						apInsertInputXML=CommonMethods.apInsert(cabinetName, sessionID, cbs_Col_name,insert_val,"NG_DPL_CBS_FILE");
						DAO_prime.DAOPrimeLogger.debug("APInsertInputXML: "+apInsertInputXML+":"+cbs_Col_name);
							
					}else{
						apInsertInputXML=CommonMethods.apInsert(cabinetName, sessionID, cbs_Col_name,insert_val,"NG_DAO_CBS_FILE");
						DAO_prime.DAOPrimeLogger.debug("APInsertInputXML: "+apInsertInputXML+":"+cbs_Col_name);
					...
					if(apInsertMaincode.equalsIgnoreCase("0"))
					{
						...
						if(WI_name.contains("DigitalPL")){
							...
							updateExternalTable("NG_DPL_EXTTABLE","isCBSRec","'Received'","WIName='"+WI_name+"'",CommonConnection.getJTSIP(),CommonConnection.getJTSPort(),CommonConnection.getCabinetName());
					...
					else
					{
						DAO_prime.DAOPrimeLogger.debug("ApInsert failed: "+apInsertMaincode);
						if(WI_name.contains("DigitalPL")){
							updateExternalTable("NG_DPL_EXTTABLE","isCBSRec","'Error'","WIName='"+WI_name+"'",CommonConnection.getJTSIP(),CommonConnection.getJTSPort(), CommonConnection.getCabinetName());
