...
//								}
				    ...
				    if (xmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec > 0)
			        {
			            ...
			            for (; objWorkList.hasMoreElements(true); objWorkList.skip(true))
			            {
			            	...
			            	CheckGridDataMap.put("Money_send_and_received_Countries_final_IBPS", objWorkList.getVal("Money_send_and_received_Countries_final_IBPS"));//pdocal-14
			            }
			        }
	    		  ...
	    		  //PDOCAL-14 money sent and received
		            String Money_send_and_received_Countries_final_IBPS=CheckGridDataMap.get("Money_send_and_received_Countries_final_IBPS");
		            
	    		    if(!("".equalsIgnoreCase(Money_send_and_received_Countries_final_IBPS)|| Money_send_and_received_Countries_final_IBPS.equalsIgnoreCase(null))){
	    		    	Notify_app_input_xml=Notify_app_input_xml.replace("#MoneySendAndReceivedCnts#",Money_send_and_received_Countries_final_IBPS);
	    		    }
	    		    else {
	    		    	Notify_app_input_xml=Notify_app_input_xml.replace("<MoneySendAndReceivedCnts>#MoneySendAndReceivedCnts#</MoneySendAndReceivedCnts>","");
	    		    }
	    		    
	    		    //ends
	    		    //PDOCAL-476 VINAYAK OFFICE ADDRESS UPDATE
	    		    
	    		    String DBQuery_addressDetails ="select Flat_Villa_No,Building_Villa_Name,Street_Location,Nearest_landmark,PO_Box_address,Emirate_City_address,address_type,Country_address  from NG_DAO_GR_ADDRESS_DETAIL with(nolock)"
	    		    		+ " where wi_name='" + processInstanceID + "'";
		            
		            String AddressDataIPXML =CommonMethods.apSelectWithColumnNames(DBQuery_addressDetails, CommonConnection.getCabinetName(), CommonConnection.getSessionID(DAONotifyAPPLog.DAONotifyAPPLogger, false));
		    	    DAONotifyAPPLog.DAONotifyAPPLogger.debug(" AddressDataIPXML: " + AddressDataIPXML);
		    	    String AddressDataOPXML = WFNGExecute(AddressDataIPXML, CommonConnection.getJTSIP(), CommonConnection.getJTSPort(), 1);
		    	    DAONotifyAPPLog.DAONotifyAPPLogger.debug("AddressDataOPXML: " + AddressDataOPXML);
		            
		    	    XMLParser AddressDataxmlParserData = new XMLParser(AddressDataOPXML);
		            
		    	    int iTotalrec_address = Integer.parseInt(AddressDataxmlParserData.getValueOf("TotalRetrieved"));
		    	    String OfficeAddress="";
		    	    if (AddressDataxmlParserData.getValueOf("MainCode").equalsIgnoreCase("0") && iTotalrec_address > 0)
		            {
		    	    	
		    	    	String xmlDataExtTab2 = AddressDataxmlParserData.getNextValueOf("Record");
		                xmlDataExtTab2 = xmlDataExtTab2.replaceAll("[ ]+>", ">").replaceAll("<[ ]+", "<");
		                // replace the spcl char above.
		                NGXmlList objWorkList2 = AddressDataxmlParserData.createList("Records", "Record");
		                String Flat_Villa_No="",Building_Villa_Name="",Street_Location="",Nearest_landmark="",PO_Box_address="",Emirate_City_address="",address_type="",Country_address="";		                
		                for (; objWorkList2.hasMoreElements(true); objWorkList2.skip(true))
		                {
		                	address_type=objWorkList2.getVal("address_type");
		                	
			                if("OFFICE".equalsIgnoreCase(address_type)){
			                	Flat_Villa_No=objWorkList2.getVal("Flat_Villa_No");
			                	Building_Villa_Name=objWorkList2.getVal("Building_Villa_Name");
			                	Street_Location=objWorkList2.getVal("Street_Location");
			                	Nearest_landmark=objWorkList2.getVal("Nearest_landmark");
			                	PO_Box_address=objWorkList2.getVal("PO_Box_address");
			                	Emirate_City_address=objWorkList2.getVal("Emirate_City_address");		                	
			                	Country_address=objWorkList2.getVal("Country_address");              	
			                	
			                	OfficeAddress = 
					    				"\n\t\t" +"<AddressDetails>"+ "\n" +
										"\t\t\t" +"<TypeOfAddress>"+address_type+"</TypeOfAddress>"+"\n" +
										"\t\t\t" +"<AddressLine1>"+Flat_Villa_No+"</AddressLine1>"+"\n" +
										"\t\t\t" +"<AddressLine2>"+Building_Villa_Name+"</AddressLine2>"+"\n" +
										"\t\t\t" +"<AddressLine3>"+Street_Location+"</AddressLine3>"+"\n" +
										"\t\t\t" +"<AddressLine4>"+Nearest_landmark+"</AddressLine4>"+"\n" +
										"\t\t\t" +"<PoBox>"+PO_Box_address+"</PoBox>"+"\n" +
										"\t\t\t" +"<EmirateCity>"+Emirate_City_address+"</EmirateCity>"+"\n" +
										"\t\t\t" +"<Country>"+Country_address+"</Country>"+"\n" +
										"\t\t" +"</AddressDetails>\n";
			                }
		                }
		            }	    
		    	    Notify_app_input_xml=Notify_app_input_xml.replace("update_OfficeAddress",OfficeAddress);
	    		    //end
