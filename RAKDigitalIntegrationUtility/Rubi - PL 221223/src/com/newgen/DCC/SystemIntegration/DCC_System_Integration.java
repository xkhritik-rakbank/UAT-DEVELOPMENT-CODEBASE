...
import com.newgen.DPL.ExternalExposure;
...
public class DCC_System_Integration implements Runnable
{
		...
		try
		{
			...
			else
			{
				...
				while(true)
				{
					...
					if("Digital_PL".equalsIgnoreCase(CommonConnection.getsProcessNameDPL()))
					{
						startDPLSysIntegration(cabinetName, UserName, jtsIP, jtsPort, sessionID, queueID, socketConnectionTimeout, integrationWaitTime, socketDetailsMap);
	...
	//Rubi for DPL
	private void startDPLSysIntegration(String cabinetName, String UserName, String sJtsIp, String iJtsPort,
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			DCCSystemIntegrationLog.DCCSystemIntegrationLogger.debug("Fetching all Workitems on DPL_SysCheckIntegration queue");
			System.out.println("Fetching all Workitems on DPL_SysCheckIntegration queue");
			...
			DCCSystemIntegrationLog.DCCSystemIntegrationLogger.debug("Number of workitems retrieved on DPL_SysCheckIntegration: "+fetchWorkitemListCount);
			System.out.println("Number of workitems retrieved on DPL_SysCheckIntegration: "+fetchWorkitemListCount);
