...
public class DCC_CIFUpdate {
			...
			HashMap<String, String> socketDetailsMap)
	{
		...
		try
		{
			...
			String DBQuery = "SELECT CIF, FATCA_Tin_Number,Tin_reason,FinalTAI,nationality FROM NG_DCC_EXTTABLE with(nolock) WHERE WI_NAME='"+processInstanceID+"'";
			...
			DCC_CIFUpdateLog.debug("CIF Update data input: "+ extTabDataIPXML);
			String extTabDataOPXML = CommonMethods.WFNGExecute(extTabDataIPXML,CommonConnection.getJTSIP(),CommonConnection.getJTSPort(),1);
			DCC_CIFUpdateLog.debug("CIF Update data output: "+ extTabDataOPXML);
