			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0){
				...
				for(int i=0; i<fetchWorkitemListCount; i++){
					...
					String decisionValue="";
					 // hritik 21.12.23 - Query change Prime 4 issue - delivery date format issue in extension call
				    String DBQuery ="select delivery_date,CardSerno,WI_name from NG_Digital_AWB_Status where WI_name='" + processInstanceID + "'";
				   ...
				   // hritik 21.12.23 -  Prime 4 issue - delivery date format issue in extension call
				    String unformattedDate=CheckGridDataMap.get("delivery_date").trim();
	                SimpleDateFormat formatterold =  new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	                ...
	                DAOExtensionLogger.DAOExtensionLogger.debug("date new :"+formattedDate);
		    		...
		    		.replace(">DATE_DEL<",">"+formattedDate+"<");
				...
				try{
					...
					if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0){
						for(int i=0; i<fetchWorkitemListCount; i++){
			                ...
			                SimpleDateFormat formmatternew = new SimpleDateFormat("dd/MM/yyyy");
			                String formattedDate=formmatternew.format(d);
			                DAOExtensionLogger.DAOExtensionLogger.debug("date new DCC :"+formattedDate);
