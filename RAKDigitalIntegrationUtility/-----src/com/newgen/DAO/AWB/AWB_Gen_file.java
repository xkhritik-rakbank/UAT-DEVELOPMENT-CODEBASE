...
public class AWB_Gen_file {
		...
		try {
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0) {
				for (int i = 0; i < fetchWorkitemListCount; i++) {
					...
					//OLD Query to by pass etb card reqsted
					String DBQuer_validateCase ="select isnull(AWB_Number,'')as 'AWB_Number' from NG_DAO_EXTTABLE with (NOLOCK ) where WI_name='" + processInstanceID + "'"
					+ "and (AWB_Number='' or"+ "AWB_Number is null) and ((is_ntb='N' and ((is_cbs_req='Y' and Is_CBS='Y') or is_cbs_req='N')) or "
					+ "(is_ntb='Y' and ((is_cbs_req='Y' and Is_CBS='Y') or is_cbs_req='N') and ((is_prime_req='Y' and Is_prime='Y') or is_prime_req='N')))";
					
				/*	String DBQuer_validateCase = "select isnull(AWB_Number,'') as 'AWB_Number' from NG_DAO_EXTTABLE with (NOLOCK ) where WI_name='"
							+ processInstanceID+ "' and (AWB_Number='' or AWB_Number is null) and "
					...
					if (xmlParserData_validateCase.getValueOf("MainCode").equalsIgnoreCase("0")
						...
						String DBQuery = "select employement_type as 'employement_type' ,Company_employer_name as 'emp_name',mobile_no_1 as 'mobNo',WI_name as 'Wi_no',Given_Name as 'first_name',Surname as 'last_name',Account_creation_date from NG_DAO_EXTTABLE with (NOLOCK) where WI_name='"
								+ processInstanceID + "'";
	...
	private void insert_ng_digital_awb_status(String Wi_name, String cabinetName, String sessionId, String sJtsIp,
		...
		try {
			...
			// ng_digital_awb_status
			final HashMap<String, String> CheckGridDataMap_awb = new HashMap<String, String>();
			String DBQuery_awb = "select WI_name,account_no,prospect_id,Given_Name,Surname,mobile_no_1,email_id_1,ChequeBk_Req,ChequeBk_ref,ECRN,AWB_Number,is_prime_req,AWB_status,is_Ntb from NG_DAO_EXTTABLE where WI_name='"
					+ Wi_name + "'";
	...
	private void MailAwbNotGeneratedDao() {
		try {
			String DBQuery1 = "Select WI_name as 'wi_name',prospect_id as 'prospect_id',concat(Given_Name,' ',Middle_Name,' '"
					...
					+ "and AWB_Number is null and Account_creation_date is not null and DATEDIFF(day,Account_creation_date,GETDATE()) >'"+AwbNotGeneratedDays+"'";
			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0) {
				for (int i = 0; i < fetchWorkitemListCount; i++) {
					...
					ColumnValueHtml = ColumnValueHtml + "<tr>" + "<td style=\"border: 1px solid black;text-align: center\">#SNO#</td>"
