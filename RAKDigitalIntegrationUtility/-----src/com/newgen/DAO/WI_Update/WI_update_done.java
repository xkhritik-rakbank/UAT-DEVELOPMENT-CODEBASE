			...
			if (fetchWorkItemListMainCode.trim().equals("0") && fetchWorkitemListCount > 0)
				{
					for(int i=0; i<fetchWorkitemListCount; i++)
					{
					    ...
					    String DBQuery_1 ="select is_Ntb,account_no,is_prime_req,is_cbs_req,event,deh_Event,deh_Workitem_status,entry_date_time,DATEDIFF(DAY,convert(datetime,entry_date_time),GETDATE()) as 'DAY_DIFF' from NG_DAO_EXTTABLE with(nolock) where wi_name='"+ processInstanceID +"'";
				    	...
				    	if("REQUEST_CUSTOMER_QUERY".equalsIgnoreCase(deh_Event) && "INP".equalsIgnoreCase(deh_Workitem_status) && no_of_day_forWi>no_of_day ){
							...
							ProcessDefId,ActivityType,decisionValue,ActivityName,EntryDATETIME);
				    	}
